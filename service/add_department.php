...
if(!(isset($_POST['id'])) and $id==$defDep){
 $sql="select * from Opm_Organ where id=$id and id>1000 and type=1";
}

else{
	$sql="select * from Opm_Organ where  parentId=$id and id>1000 and type=1";
}
...
function has_child($id){
	$logger = Logger :: getRootLogger();
	mysql_select_db("IOV_demo");
	$query="select count(*)  from Opm_Organ where parentId=$id and id>1000 and type=1";

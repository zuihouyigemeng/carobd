...
if ($opType == 1 or $opType == '1') {
	...
	$query = "select ua.userID,ua.email,ua.username,ua.smsNum,ua.fullname,role.name as roleName,role.id as roleId,oo.name as depName from  Users_Admin  ua,Opm_User_Role our,Opm_Role role,Opm_Organ oo  where ua.userID=our.userID and our.roleId=role.id and ua.organId=$depID and ua.isCustomer=0 and our.organId=oo.id";
...
if ($opType ==3 or $opType == '3') {
	mysql_select_db("IOV_demo");
	$checkUnameSql = "select count(*) as num from Users_Admin  where username='$username'  ";
	...
	$smsNum1=substr($smsNum,5,6);
	$passw = encrypt2($smsNum1,0);	
	$sql="INSERT INTO Users_Admin (`username`, `password`, `acctID`, `dealerID`, `language`, `smsNum`, `organId`, `fullname`, `email`) VALUES ('$username', '$passw', 'CT_db', 0, 'cn', '$smsNum', '$depID', '', '$email')";
	...
	$sql="insert into Opm_User_Role  (userID,roleId,organId) values ('$userID','$roleId','$depID')";
	$logger->debug("----------------sql:------" . $sql);
	if (!mysql_query($sql)) {
			echo 1001;
		} else {
...
if ($opType ==4 or $opType == '4') {
	...
	$sql = array ();
	$sql[]="update  Users_Admin set userName='$username',email='$email',smsNum='$smsNum',organId='$depID'  where userID='$userID'";

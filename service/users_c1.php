...
if ($opType == 1 or $opType == '1') {
	...
	$query = "select ua.userID,ua.email,ua.username,ua.smsNum,ua.fullname,role.name as roleName,role.id as roleId, oo.name as depName". 
		...
		" Opm_Role role,".
		" Customers oo  ".
		" where ua.userID=ou.userID and ou.roleId=role.id and ou.organId=$depID and ou.organId=oo.id and ua.iscustomer=1";
...
if ($opType ==3 or $opType == '3') {
	...
	$passw = encrypt2($smsNum,0);	
	$sql="INSERT INTO Users_Admin (`username`, `password`, `acctID`, `dealerID`, `language`, `smsNum`, `organId`, `fullname`, `email`,`isCustomer`) VALUES ('$username', '$passw', 'IOV_demo', 0, 'cn', '$smsNum', 0, '', '$email',1)";
